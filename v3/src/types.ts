 ...
 * |------|-------|-----------|
 * | **Type 1** | `typeof === 'function' && length === 2` | `Component<any>` |
 * | **Type 2** | `typeof === 'object' && has .then` | `Promise<...>` |
 * | **Type 3** | `typeof === 'function' && length === 0` | `() => Promise<...>` |
 *
 * @example
 *
 * ```ts
...
export type ComponentTypeKind<T> =
  T extends Component<any> ? "component"
  : T extends Promise<any> ? "promise"
  : T extends () => Promise<any> ? "loader"
  : never;

export type UnwrapRouteComponentPromise<T> =
  T extends Promise<infer M> ? M extends { default: infer C } ? C : M
  : never;
export type UnwrapRouteComponentFunction<T> =
  T extends () => Promise<infer M> ? M extends { default: infer C } ? C : M
  : never;


export type UnwrapRouteComponent<T> = ComponentTypeKind<T> extends "component" ? Component<Record<string, any>>
  : ComponentTypeKind<T> extends "promise" ? UnwrapRouteComponentPromise<T>
  : ComponentTypeKind<T> extends "loader" ? UnwrapRouteComponentFunction<T>
  : never;

export type RouteComponent =
  // Straight Svelte component (rendering ready, no importing required).
  | Component<any>
  // Promise that resolves to a component (must import synchronously).
  | Promise<{ default: Component<any> } | Component<any>>
  // Async function that returns a component (must import asynchronously, lazy loading).
  | (() => Promise<{ default: Component<any> } | Component<any>>);
 ...
 * @category Configuration
 */
export interface BaseRouteConfig <T>{
  /**
   * Route path pattern.
   *
 ...
 */
export interface SimpleRouteConfig<T=unknown> extends BaseRouteConfig<T> {
  /**
 ...
 */
export interface EnhancedRouteConfig<T = unknown> extends SimpleRouteConfig<T> {
  /**
  ...
  hooks?: {
     ...
     * cancels navigation.
     */
    beforeEnter?: HookFunction<T> | HookFunction<T>[];
     ...
     */
    afterEnter?: HookFunction<T> | HookFunction<T>[];
     ...
     * cancels navigation.
     */
    beforeLeave?: HookFunction<T> | HookFunction<T>[];
     ...
     */
    afterLeave?: HookFunction<T> | HookFunction<T>[];
  };
}
 ...
 */
export interface AdvancedRouteConfig<T> extends EnhancedRouteConfig<T> {
  /**
   ...
   */
  matcher?: (path: string) => RouteMatch<T> | null;
 ...
 * @category Guards
 */
export type GuardFunction<T=unknown> = (context: GuardContext<T>) => boolean | Promise<boolean>;
