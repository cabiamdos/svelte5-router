...
describe('Dashboard Component', () => {
	...
	describe('Content Rendering', () => {
		test('renders user name safely', async () => {
			...
			await expect.element(welcomeText).toHaveTextContent('Welcome, <script>alert("xss")</script>!');
		});
	...
	describe('Edge Cases', () => {
		test('handles very long user names', async () => {
			...
			const component = page.getByTestId('dashboard-component');
			const welcomeText = component.getByText(/Welcome,/);
			await expect.element(welcomeText).toHaveTextContent(`Welcome, ${longName}!`);
		});
		...
		test('handles user names with newlines', async () => {
			...
			const component = page.getByTestId('dashboard-component');
			const welcomeText = component.getByText(/Welcome,/);
			await expect.element(welcomeText).toHaveTextContent('Welcome, First Last!');
		});
	});
	...
	describe('Component Structure', () => {
		test('has correct test id', async () => {
			...
			const component = page.getByTestId('dashboard-component');
			await expect.element(component).toHaveAttribute('data-testid', 'dashboard-component');
		});
