<script lang="ts">
	...
	import { Router, createAuthGuard, goto, loggingMiddleware } from '@v3/api/enhanced';
	import type { EnhancedRouteConfig, MiddlewareFunction } from '@v3/types';
	...
	const routes: EnhancedRouteConfig[] = [
		{
			path: '/',
			component: Home,
			hooks: {
...
<div data-testid="enhanced-api-test">
	...
	<nav data-testid="navigation">
		<button data-testid="nav-home" onclick={() => goto('/enhanced/')}>Home</button>
		<button data-testid="nav-about" onclick={() => goto('/enhanced/about')}>About</button>
		<button data-testid="nav-dashboard" onclick={() => goto('/enhanced/dashboard')}
			>Dashboard</button
		>
		<button data-testid="nav-login" onclick={() => goto('/enhanced/login')}>Login</button>
	</nav>
