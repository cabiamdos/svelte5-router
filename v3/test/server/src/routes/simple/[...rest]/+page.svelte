<script lang="ts">
	...
	import { SimpleRouter, goto, type SimpleRouteConfig } from '@v3/api/simple';
	...
	const routes: SimpleRouteConfig[] = [
		...
		{ path: '/users/:userId/posts/:postId', component: async () => import('$lib/Posts.svelte') },
...
<div data-testid="simple-api-test">
	...
	<nav data-testid="navigation">
		...
		>
		<button data-testid="nav-files" onclick={() => goto('/simple/files/docs/test.pdf')}
			>Files</button
		>
	</nav>

	<div data-testid="router-content">
		<SimpleRouter {routes} basePath="/simple" currentUrl={page.url.pathname} />
	</div>
</div>
